...
class Game:
    ...
    def handle_input_p(self, display_p):
        ...
        if display_p is not None:
            if self.state == 'play':
                ...
                self.adjust_ai(display_p)
        if self.state == 'play' or self.state == 'wait':
    ...
    def _am(self):
        move = self.board.get_best_move(think_time=1000)
        self.apply_move(move)

    def auto_move(self):
        self.state = 'wait'
        Thread(target=self._am).start()
    ...
    def apply_move(self, move):
        ...
        if not self.board.get_pms():
            self.state = 'play'
            return
        if ((self.ai_chn and ' w ' in self.board.fen) or
            (self.ai_int and ' b ' in self.board.fen)):
            self.auto_move()
        else:
            self.state = 'play'

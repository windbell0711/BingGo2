...
main_menu =  [
             Button(['self.state = \'play\''],['返回游戏'], rect=(0.25,0.9,0.2,0.05), shade_time_max=1),
             Button(['self.ai_think_time = 1', 'self.ai_think_time = 40', 'self.ai_think_time = 1000', 'self.ai_think_time = 3000'],
                    ['人机:新手', '人机:入门', '人机:高级', '人机:大师']),#!
             Button(['self.hint_think_time = 1000', 'self.hint_think_time = 3000'],
                    ['提示:高级', '提示:大师']),
             Button(['self.save()'],['保存']),
             Button(['self.load()'],['载入'], shade_time_max=1),
             Button(['self.reset();self.state = \'play\''],['新局'], rect=(0.55,0.9,0.2,0.05), shade_time_max=1)
             ]
...
class Game:
    ...
    def handle_input_p(self, display_p ,w, h, mx, my, c):
        if self.state == 'play':
    ...
    def save(self):
        ...
        file_name = f'saves\\{formatted_time}.binggo'
